...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // use relative manifest path so the site works as a user/organization page
  ...
  function loadAndRenderCSV(csvPath, containerEl){
    ...
    // normalize path: allow leading / or relative paths
    let normalizedPath = csvPath;
    if (normalizedPath.startsWith('/')) normalizedPath = '.' + normalizedPath;

    fetch(normalizedPath)
      .then(r => {
        if (!r.ok) throw new Error('Failed to fetch ' + normalizedPath);
        return r.text();
      })
      .then(text => {
        ...
        if (parsed.errors && parsed.errors.length) {
          console.warn('CSV parse warnings/errors for', normalizedPath, parsed.errors);
        }
